...
def render_form(title, action, fields, method=Ellipsis, enctype=Ellipsis):
    ...
    for record in fields:
        ...
        raw_ec = record[4] if len(record) > 4 else None
        extra_classes = Markup(' ' + raw_ec) if raw_ec is not None else ''
        ...
        if ftype in ('submit', 'reset', 'button'):
            ...
            result.append(
                Markup('  <p class="mb-3%s"%s>%s</p>') %
                    (extra_classes, maybe_attr('id', name), label)
            )

        elif ftype == 'hidden':
            result.append(
                Markup('  <input type="hidden"%s%s%s/>') %
                    (maybe_attr('class', raw_ec), maybe_attr('id', name),
                     value_attrs)
            )
